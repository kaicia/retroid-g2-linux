...
#   2. copy kernel/g2/dts/qcom/g2-cliffs-sd.dts into arch/arm64/boot/dts/qcom/
#   3. ensure "dtb-\$(CONFIG_ARCH_QCOM) += g2-cliffs-sd.dtb" in the qcom Makefile
#   4. make defconfig; apply kernel/g2/config/linux.aarch64.conf fragment
#   5. make Image dtbs  (G2 DTB built by the kernel DT build system)
#   6. optional: busybox minimal rootfs (scripts/build-g2-rootfs.sh)
#   7. optional: microSD image (scripts/prepare-g2-sd-image.sh --kernel --dtb --rootfs)
#   8. non-destructive validation (scripts/validate-g2-sd-artifacts.sh)
...
for t in curl tar xz make gcc sha256sum; do need "$t"; done
...
# --- 2. integrate the G2 DTS into the real kernel DT build target ----------
...
echo ">> configuring kernel (defconfig + G2 fragment)"
kmake defconfig >/dev/null
if [ -s "$ROOT/kernel/g2/config/linux.aarch64.conf" ]; then
    cp "$ROOT/kernel/g2/config/linux.aarch64.conf" "$KBUILD/kernel-frag.conf"
